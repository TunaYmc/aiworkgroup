            ...
            try:
                while turns < max_turns:
                    ...
                    for tc in tool_calls:
                        ...
                        # Execute with strict authorization & live event streaming
                        event_queue = asyncio.Queue()

                        async def emit_tool_thought(thought_content: str):
                            await event_queue.put({"type": "thought", "content": thought_content})

                        tool_context = dict(context)
                        tool_context["thought_emitter"] = emit_tool_thought

                        exec_task = asyncio.create_task(
                            tool_registry.execute_tool(tool_name, tool_args, tool_context)
                        )
                        ...
                        while not exec_task.done():
                            try:
                                live_event = await asyncio.wait_for(event_queue.get(), timeout=keepalive_interval)
                                yield live_event
                            except asyncio.TimeoutError:
                                elapsed_seconds += int(keepalive_interval)
                                if tool_name == "browser":
                                    yield {
                                        "type": "thought",
                                        "content": f"🌐 Tarayıcı sayfayı işlemeye devam ediyor ({elapsed_seconds}s)..."
                                    }
                                else:
                                    yield {"type": "ping"}

                        # Drain any remaining events generated by the tool
